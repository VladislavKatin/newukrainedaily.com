...
(async () => {
  ...
  try {
    ...
    process.stdout.write(`
Production deploy, warm-up, and health check passed in ${durationSeconds}s.
`);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await notifyTelegram(
      "error",
      "Сбой деплоя production",
      `Не удалось завершить deploy, прогрев или health-check. Проверьте Vercel, warm-up и состояние production.\n\nДетали: ${message}`
    );
    process.stderr.write(`
